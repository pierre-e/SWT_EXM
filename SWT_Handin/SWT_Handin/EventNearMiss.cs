...
namespace SWT_Handin
{
    public class EventNearMiss : IEvent
    {
        ...
        {
            var nearMissList = new List<EventMessage>();
            foreach (ITrack track1 in tracks)
            {
                foreach (ITrack track2 in tracks)
                {
                    ...
                    if (distanceBetweenTracks < _atc.NearMissDist)
                    {
                        nearMissList.Add(new EventMessage(new List<ITrack> {track1, track2}, GetType()));
                    }
                }
            }
            return nearMissList;
